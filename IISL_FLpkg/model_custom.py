...
class CustomModelList(list):
    def fed_avg(self, x, y, metric, central_server):
      ...
      for i, model in enumerate(self):
        train_results = model.train_step(x[i], y[i], metric)
    ...
    def fed_prox(self, x, y, metric, central_server):
      trainable_vars = central_server.model.trainable_variables
      loss_avg = 0
      sca_metric_avg = 0
      for i, model in enumerate(self):
        train_results = model.train_step_prox(x[i], y[i], metric, trainable_vars)
        loss_avg += train_results[1]
        sca_metric_avg += train_results[2]
        # Averaging gradients
        if(i % len(self) == 0):
